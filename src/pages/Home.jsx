...
// Two new home-page sections built to match the existing Krishna Accessories
// design language (same card grid, pill filters, spacing, and hover language
// already used in the "Selected Editions" block of Home.jsx).
//
// 1) BestSellersSection  -> ranks products by rating (then discount) and
//    shows the top performers with a "Bestseller" ribbon.
// 2) ShopByPriceSection  -> four price-band tabs (Under ₹5k/10k/15k/20k)
//    that filter the live product list, same interaction pattern as the
//    category pills you already have.
//
// HOW TO WIRE THESE IN (Home.jsx):
//   import { BestSellersSection, ShopByPriceSection } from '../components/HomeExtraSections';
//   ...
//   <NewArrivalsSection products={products} onToast={setToastMessage} />
//   <BestSellersSection products={products} onAddToCart={handleAddToCart} onBuyNow={handleBuyNow} />
//   <ShopByPriceSection products={products} onAddToCart={handleAddToCart} onBuyNow={handleBuyNow} />
//   <WhyChooseUsSection />
...
// 1) BEST SELLERS
// ============================================================
export function BestSellersSection({ products = [], onAddToCart, onBuyNow, limit = 8 }) {
  ...
  return (
    <section className="bg-[#FAFAFB] py-10 sm:py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-gray-500 max-w-md">
              The pieces our customers keep coming back for, ranked by rating and demand.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-5 lg:gap-6">
          {bestSellers.map((product, idx) => (
            <div key={product.id} className="relative">
              {idx < 3 && (
                <span className="absolute top-2.5 left-2.5 z-10 inline-flex items-center gap-1 rounded-full bg-gray-950/90 backdrop-blur-md px-2.5 py-1 text-[10px] font-semibold text-white shadow-2xs">
                  🔥 #{idx + 1} Bestseller
                </span>
              )}
              <ProductCard
...
export function ShopByPriceSection({ products = [], onAddToCart, onBuyNow, limit = 8 }) {
  const [activeBand, setActiveBand] = useState(priceBands[0]);

  const filtered = useMemo(() => {
    return products
      .filter((p) => Number(p.price) <= activeBand.max)
      .sort((a, b) => Number(b.price) - Number(a.price))
      .slice(0, limit);
  }, [products, activeBand, limit]);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-gray-500 max-w-md">
              Find something great, whatever your budget.
            </p>
          </div>

          <Link
            to={`/shop?maxPrice=${activeBand.max}`}
            className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-gray-900 hover:text-black hover:underline self-start md:self-auto group shrink-0"
          >
            <span>View All {activeBand.label}</span>
            <ArrowRightIcon className="w-4 h-4 transition-transform group-hover:translate-x-1" />
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-6 sm:mb-8 no-scrollbar">
          {priceBands.map((band) => {
            const isActive = activeBand.max === band.max;
            return (
              <button
                key={band.label}
                type="button"
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-12 rounded-2xl bg-gray-50 border border-gray-200/80">
            <p className="text-sm font-semibold text-gray-700">No products found in {activeBand.label}.</p>
            <button
              type="button"
              onClick={() => setActiveBand(priceBands[priceBands.length - 1])}
              className="mt-3 text-xs font-bold text-black underline cursor-pointer"
            >
              Try Under ₹20,000
            </button>
