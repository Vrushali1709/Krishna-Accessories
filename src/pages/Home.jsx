...
// Hero carousel slides — replace image URLs with your own product photos anytime
const watchSlides = [
  {
    image: 'https://images.unsplash.com/photo-1524805444758-089113d48a6d?w=1200',
    name: 'Titan Edge Chronograph',
    tag: 'Swiss Movement • Sapphire Crystal'
  },
  {
    image: 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=1200',
    name: 'Rolex Oyster Perpetual',
    tag: 'Stainless Steel • Automatic'
  },
  {
    image: 'https://images.unsplash.com/photo-1547996160-81dfa63595aa?w=1200',
    name: 'Fossil Gen Smartwatch',
    tag: 'Titanium Case • Always-On Display'
  },
];
...
export default function Home() {
  ...
  // Auto-rotate hero watch carousel every 3 seconds
  useEffect(() => {
    const timer = setInterval(() => {
      setActiveSlide((prev) => (prev + 1) % watchSlides.length);
    }, 3000);
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#FDFBF7] via-[#FAF9F5] to-white border-b border-gray-200/80">
        <div className="mx-auto flex min-h-[460px] lg:min-h-[520px] max-w-7xl items-center px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-center w-full">
            ...
            {/* Right Column: Watch Carousel (3 watches, auto-rotating) */}
            <div className="lg:col-span-7 relative animate-fade-in w-full">
              <div className="rounded-2xl sm:rounded-3xl border border-gray-200 bg-white p-2 sm:p-2.5 shadow-xl group">
                <div className="relative overflow-hidden rounded-xl sm:rounded-2xl aspect-[4/3] sm:aspect-[16/10]">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setActiveSlide((prev) => (prev - 1 + watchSlides.length) % watchSlides.length)
                    }
                    className="absolute left-2 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-full bg-white/80 text-gray-900 shadow-md hover:bg-white transition z-10"
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveSlide((prev) => (prev + 1) % watchSlides.length)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-full bg-white/80 text-gray-900 shadow-md hover:bg-white transition z-10"
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                    {watchSlides.map((_, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => setActiveSlide(idx)}
                        className={`h-1.5 rounded-full transition-all ${
                          idx === activeSlide ? 'w-5 bg-white' : 'w-1.5 bg-white/50'
                        }`}
                ...
                {/* Caption Bar — updates with the active watch slide */}
                <div className="mt-2 sm:mt-2.5 flex items-center justify-between gap-2.5 sm:gap-4 rounded-xl bg-[#FAF9F6] border border-gray-200/80 px-3 sm:px-4 py-2 sm:py-2.5">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <span className="flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full bg-amber-50 border border-amber-200/60 text-amber-600 text-xs font-bold shrink-0">
                      ★
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] sm:text-[11.5px] font-bold uppercase tracking-wider text-gray-900 truncate">
                        {watchSlides[activeSlide].name}
                      </p>
                      <p className="text-[9px] sm:text-[10px] text-gray-500 truncate">
                        {watchSlides[activeSlide].tag}
                      </p>
                    </div>
                  </div>
                  <span className="hidden sm:inline-flex text-[9.5px] font-bold text-[#B89758] uppercase tracking-wider bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20 shrink-0">
