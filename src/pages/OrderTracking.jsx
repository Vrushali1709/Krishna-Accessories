        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
          <Reveal effect="fade-up">
            ...
            <p className="mt-2 text-xs sm:text-sm text-gray-500 max-w-md mx-auto">
              Enter your consignment code to view real-time transit milestones from Mumbai flagship dispatch to doorstep delivery.
            </p>

            {/* Search Form */}
            <form onSubmit={handleSearch} className="mt-6 max-w-md mx-auto flex flex-col sm:flex-row gap-2.5">
              <div className="relative flex-1 min-w-0">
                <input
                  ...
                  className="w-full rounded-full border border-gray-200 bg-[#F4F4F6] px-5 py-3 text-xs text-gray-900 font-mono uppercase outline-none focus:border-[#8C6734] focus:bg-white transition shadow-2xs"
                  required
                />
              </div>
              <button
                type="submit"
                className="inline-flex items-center justify-center gap-2 rounded-full bg-[#111827] px-7 py-3 text-xs font-bold uppercase tracking-wider text-white hover:bg-black transition shadow-sm shrink-0 cursor-pointer hover:-translate-y-0.5"
              >
            ...
            {/* Sample ID Chips */}
            <div className="mt-3.5 flex items-center justify-center gap-2 text-xs text-gray-400 flex-wrap">
              <span>Quick demo references:</span>
              {sampleOrderIds.map((sid) => (
                <button
                  ...
                  className="font-mono text-gray-900 font-bold hover:text-[#8C6734] transition hover:underline cursor-pointer"
                >
                  {sid}
                </button>
              ))}
        ...
        {isSearching ? (
          ...
          <Reveal effect="fade-up">
            <div className="rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-sm">
              <h3 className="text-base font-serif font-bold text-gray-950">Consignment Reference Not Found</h3>
              <p className="mt-2 text-xs text-gray-500">
                No matching consignment was found for code <strong className="text-gray-900 font-mono">{orderIdInput}</strong>. Please check your order invoice or contact our Mumbai concierge desk.
              </p>
            </div>
          </Reveal>
        ) : activeOrder ? (
          <div className="space-y-6">
              ...
              <div className="rounded-3xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-5">
                  <div className="min-w-0">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8C6734]">Consignment ID</span>
                    <h2 className="text-xl sm:text-2xl font-bold font-mono text-gray-950 truncate mt-0.5">
                      {activeOrder.id}
                    </h2>
                    <p className="text-xs text-gray-500 mt-1 truncate">Placed on {activeOrder.date} &bull; {activeOrder.paymentMethod}</p>
                  </div>

                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`rounded-full border px-4 py-1.5 text-xs font-bold tracking-wide ${activeOrder.status === 'Delivered'
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-5 border-b border-gray-100 text-xs">
                  ...
                  <div className="min-w-0">
                    <span className="text-gray-400 block font-semibold text-[11px]">Delivery City:</span>
                    <span className="font-bold text-gray-900 mt-0.5 block truncate">
                ...
                <div className="pt-6">
                  <h3 className="text-xs font-bold uppercase tracking-[0.16em] text-gray-950 mb-6">
                    ...
                    {activeOrder.timeline && activeOrder.timeline.map((step, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative group">
                          ...
                          {/* Step Indicator Node */}
                          <span
                            className={`absolute -left-[31px] top-0.5 flex h-4 w-4 items-center justify-center rounded-full border-2 bg-white transition-transform duration-200 group-hover:scale-125 ${isCancelledNode
            ...
            <Reveal effect="fade-up" delay={100}>
              <div className="rounded-3xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm">
                <h3 className="text-xs font-bold uppercase tracking-[0.16em] text-gray-950 mb-4">
                    ...
                    <div key={idx} className="py-3 flex items-center justify-between gap-3 text-xs min-w-0">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-[11px] text-gray-400 truncate block">{it.brand} &bull; Qty: {it.quantity} {it.color && `&bull; ${it.color}`}</span>
                        </div>
                      </div>
