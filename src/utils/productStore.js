...
export function getProductOptionValues(options) {
  if (!Array.isArray(options)) return [];
  return uniqueValues(options.map(option => {
    if (typeof option === 'string') return option;
    return option?.value || option?.label || option?.name || option?.size || '';
  }));
}

function getVariantRecords(product) {
  const records = [
    ...(Array.isArray(product?.variantDetails) ? product.variantDetails : []),
...
export function getProductGallery(product) {
  ...
  const images = uniqueValues(storedImages);

  if (!images.length) return [];

  // Older catalog records have one or two photos. These crops keep the actual
  // product visible while giving the gallery useful front, detail, and profile views.
  const cropVariants = [
    'center',
    'faces',
    'entropy',
  ...
  while (images.length < 4) {
    const source = images[cropIndex % images.length];
    const separator = source.includes('?') ? '&' : '?';
    const crop = cropVariants[images.length % cropVariants.length];
    const derivedImage = `${source}${separator}auto=format&fit=crop&crop=${crop}&w=900&v=${images.length}`;
    if (!images.includes(derivedImage)) images.push(derivedImage);
    cropIndex += 1;
  }
...
export function getProductSelection(product, color = '', variant = '') {
  if (!product) return null;

  const records = getVariantRecords(product);
  const selectedRecord = records.find(record => {
    const recordColor = record.color || record.colour || '';
    const recordVariant = record.variant || record.size || record.value || record.name || '';
    return (!recordColor || recordColor === color) && (!recordVariant || recordVariant === variant);
  });

  if (!selectedRecord) {
    const colorValues = getProductOptionValues(product.colors);
    const variantValues = getProductOptionValues(product.variants);
    const selectedIndex = Math.max(colorValues.indexOf(color), variantValues.indexOf(variant));
    const sourceImages = uniqueValues([
      product.image,
      ...(Array.isArray(product.images) ? product.images : []),
      ...(Array.isArray(product.gallery) ? product.gallery : [])
    ].filter(Boolean));

    if (selectedIndex > 0 && sourceImages.length > 1) {
      const image = sourceImages[selectedIndex % sourceImages.length];
      return { ...product, image, images: [image, ...sourceImages.filter(source => source !== image)] };
    }
    return product;
  }

  const price = selectedRecord.price !== undefined
    ? Number(selectedRecord.price)
    : Number(product.price) + (Number(selectedRecord.priceModifier) || 0);
  const selectedImages = selectedRecord.images || (selectedRecord.image ? [selectedRecord.image] : undefined);

  return {
    ...product,
    ...selectedRecord,
    ...(selectedImages ? { images: selectedImages, image: selectedImages[0] } : {}),
    ...(Number.isFinite(price) ? { price } : {}),
    ...(selectedRecord.stock !== undefined ? { stock: Number(selectedRecord.stock) } : {}),
    ...(selectedRecord.sku ? { sku: selectedRecord.sku } : {}),
