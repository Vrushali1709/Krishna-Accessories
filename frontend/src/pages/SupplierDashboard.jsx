...
export default function SupplierDashboard() {
  ...
  const handleNotificationClick = (notif) => {
    ...
    if (notif.targetSection === 'catalog') {
  ...
  // Active Section title for breadcrumbs
  const currentSectionTitle = activeSection === 'catalog' ? 'Catalog' : 'Dashboard';
    ...
    <div className="h-screen max-h-screen w-full bg-[#F9F9F8] text-zinc-900 flex overflow-hidden font-sans selection:bg-zinc-900 selection:text-white">
      ...
      >
        ...
        <div className="flex flex-col min-h-0 flex-1 overflow-hidden">
          ...
          {/* Navigation Buttons (Direct Dashboard & Catalog Only) */}
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 font-medium transition-all cursor-pointer ${isActive
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    {!sidebarCollapsed && (
                      <span className="truncate text-left text-xs font-medium">{item.title}</span>
                    )}
                  </div>

                  {!sidebarCollapsed && item.badge && (
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${isActive ? 'bg-zinc-700 text-zinc-200' : 'bg-zinc-800 text-zinc-400'
                      }`}>
                      {item.badge}
                    </span>
                  )}
      ...
      >
        ...
        <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between border-b border-zinc-200/80 bg-white/95 px-4 sm:px-6 lg:px-8 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-xs text-zinc-500 truncate min-w-0 font-medium">
              ...
              <span className="text-zinc-900 font-semibold truncate">{currentSectionTitle}</span>
            </div>
          </div>
        ...
        <main className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full flex-1">
          ...
          {/* VIEW SECTION 1: DASHBOARD (Overview, Fulfillment Queue, Inventory Control, Financials) */}
            ...
            <div className="space-y-6">
              ...
              {pendingOrdersCount > 0 && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-amber-200/80 bg-amber-50/70 p-5 shadow-2xs">
                  ...
                  <button
                    onClick={() => {
                      const el = document.getElementById('fulfillment-queue-section');
              ...
              <div id="fulfillment-queue-section" className="space-y-4 rounded-2xl border border-zinc-200/80 bg-white p-5 sm:p-6 shadow-2xs">
                  ...
                  <div className="flex flex-wrap items-center gap-2.5">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-8 pr-3 py-1.5 text-xs rounded-xl border border-zinc-200 bg-zinc-50 placeholder:text-zinc-400 outline-none focus:bg-white focus:border-zinc-400 w-48 sm:w-56 transition shadow-2xs"
                      />
                      {orderSearch && (
                    ...
                    <select
                      ...
                      className="rounded-xl border border-zinc-200 bg-zinc-50 px-2.5 py-1.5 text-xs font-semibold text-zinc-700 outline-none focus:bg-white focus:border-zinc-400 cursor-pointer shadow-2xs"
                    >
                          ...
                          <tr key={order.id} className="hover:bg-zinc-50/60 transition">
                            ...
                            <td className="py-3.5 px-4 text-right">
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-xl bg-zinc-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-black transition shadow-2xs cursor-pointer"
                              >
              ...
              {/* Inventory Stock Health & Quick Control */}
              <div className="space-y-4 rounded-2xl border border-zinc-200/80 bg-white p-5 sm:p-6 shadow-2xs">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-100 pb-4">
                  <div>
                    <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-900">
                      Inventory Stock Control & Valuation
                    </h3>
                    <p className="text-xs text-zinc-400 mt-0.5">Quick adjust physical stock units and replenish catalog inventory on demand.</p>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {supplierProducts.map(p => (
                    <div key={p.id} className="rounded-xl border border-zinc-200/80 bg-zinc-50/50 p-4 shadow-2xs flex flex-col justify-between hover:border-zinc-300 hover:bg-white transition">
                      <div>
                        <div className="flex items-center gap-3">
                          <img
                            src={p.image || p.images?.[0]}
                            alt={p.name}
                            className="h-11 w-11 rounded-xl object-cover bg-white border border-zinc-200/80 p-0.5 shrink-0"
                          />
                          <div className="min-w-0">
                        ...
                        <div className="mt-3 flex items-center justify-between rounded-lg bg-white p-2.5 border border-zinc-200/70">
                          <span className="text-xs text-zinc-500 font-medium">Available Stock:</span>
                          <span className={`text-xs font-bold font-mono ${Number(p.stock) === 0 ? 'text-rose-600' : Number(p.stock) < 10 ? 'text-amber-600' : 'text-emerald-700'}`}>
                            {p.stock} Units
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          ...
                          className="flex-1 rounded-lg border border-zinc-200 bg-white py-1 text-xs font-semibold text-zinc-700 hover:bg-zinc-100 transition cursor-pointer shadow-2xs"
                          title="Decrease by 1"
                        >
                          −1
                        </button>
                        <button
                          ...
                          className="flex-1 rounded-lg border border-zinc-200 bg-white py-1 text-xs font-semibold text-zinc-800 hover:bg-zinc-100 transition cursor-pointer shadow-2xs"
                          title="Add 5 units"
                        >
                          +5
                        </button>
                        <button
                          ...
                          className="flex-1 rounded-lg bg-zinc-900 py-1 text-xs font-semibold text-white hover:bg-black transition shadow-2xs cursor-pointer"
                          title="Add 20 units"
                        >
                          +20
                        </button>
              ...
              <div className="rounded-2xl border border-zinc-200/80 bg-white p-5 sm:p-6 shadow-2xs space-y-5">
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-900">
                    Vendor Financial Settlement Breakdown
                  </h3>
          ...
          {/* VIEW SECTION 2: CATALOG (Products & Add / Edit / Delete) */}
          {activeSection === 'catalog' && (
            <div className="space-y-5">
              ...
              {lowStockItems.length > 0 && (
                <div className="flex items-center justify-between rounded-xl border border-amber-200/80 bg-amber-50/80 p-3.5 text-xs text-amber-900">
                  <div className="flex items-center gap-2.5">
                    <AlertTriangle className="h-4 w-4 text-amber-600 shrink-0" />
                    <span><strong>{lowStockItems.length} items</strong> have low inventory (&lt; 10 units). Restock recommended.</span>
                  </div>
                  <button
                    onClick={() => setProductStockFilter('Low Stock')}
