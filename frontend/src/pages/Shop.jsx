...
export default function Shop() {
  ...
  // Sidebar Accordion Section Toggles
  ...
  // "See More" toggle for Brand list
  ...
  // Sync Products from custom events / localStorage
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] text-gray-900 font-sans antialiased overflow-x-clip selection:bg-amber-100 selection:text-amber-900">
      <Navbar />
      ...
      {/* ================= 1. CLEAN BREADCRUMB & LOCATION STRIP ================= */}
      <section className="bg-white border-b border-gray-200/75 py-2.5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs">
          {/* Breadcrumb Trail */}
          <nav className="flex items-center gap-1.5 text-neutral-400 font-medium overflow-x-auto no-scrollbar">
            <Link to="/" className="hover:text-black transition shrink-0">
              Home
            </Link>
            ...
            <button
              ...
              className={`hover:text-black transition cursor-pointer shrink-0 ${
                category === 'All' ? 'font-bold text-black' : ''
              }`}
            >
              Shop All
            </button>
            ...
            {selectedBrand !== 'All' && (
              <>
                <ChevronRight className="w-3 h-3 text-neutral-300 shrink-0" />
                <span className="font-bold text-[#B89758] shrink-0">{selectedBrand}</span>
              </>
            )}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-4 text-[11px] text-gray-500 font-medium">
            ...
            <span className="flex items-center gap-1">
              ...
              BlueDart Insured Express
            </span>
          </div>
        </div>
      </section>

      {/* ================= 2. MAIN CATALOGUE LAYOUT ================= */}
      <main
        ref={productsTopRef}
        className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-5 sm:py-7"
      >
        <div className="grid gap-6 lg:grid-cols-[260px_1fr] items-start">
          
          {/* ================= LEFT SIDEBAR (TITAN-STYLE ACCORDION FILTERS) ================= */}
          <aside className="hidden lg:block">
            <div className="space-y-4">
              
              {/* SORT BY DROPDOWN (Titan reference position above filters) */}
              <div className="rounded-xl border border-gray-200 bg-white p-3 shadow-xs">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none rounded-lg border border-gray-200 bg-[#F9FAFB] py-2 pl-3 pr-8 text-xs font-bold text-gray-900 outline-none focus:border-black transition cursor-pointer"
                  >
              ...
              {/* FILTER BY CONTAINER */}
              <div className="rounded-xl border border-gray-200/90 bg-white shadow-xs overflow-hidden divide-y divide-gray-100">
                
                {/* Filter By Header with CLEAR ALL */}
                <div className="flex items-center justify-between px-4 py-3.5 bg-[#FAF9F6]">
                  <div className="flex items-center gap-2">
                    ...
                    {activeFiltersCount > 0 && (
                      <span className="flex h-4 min-w-4 px-1 items-center justify-center rounded-full bg-amber-500 text-[10px] font-black text-white">
                        {activeFiltersCount}
                      </span>
                    )}
                  </div>
                  ...
                  {activeFiltersCount > 0 && (
                    <button
                      ...
                      className="text-[11px] font-bold text-rose-600 hover:text-rose-800 uppercase tracking-wider hover:underline cursor-pointer"
                    >
                      Clear All
                    </button>
                  )}
                ...
                {/* 1. BRANDS ACCORDION (With Checkboxes & See More v) */}
                <div className="p-4">
                  <button
                    type="button"
                    onClick={() => toggleSection('brands')}
                    className="w-full flex items-center justify-between text-left font-bold text-xs uppercase tracking-wider text-gray-900 hover:text-amber-800 transition cursor-pointer"
                  >
                    <span className="flex items-center gap-1.5">
                      Brands
                      {selectedBrand !== 'All' && (
                        <span className="text-[10px] lowercase font-semibold text-[#B89758]">
                          ({selectedBrand})
                        </span>
                      )}
                    </span>
                    <span className="text-gray-400 text-sm">
                      {openSections.brands ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                    </span>
                  </button>

                  {openSections.brands && (
                    <div className="mt-3 space-y-2 animate-fade-in">
                      {/* Brand Search Box if more than 6 brands */}
                      {dynamicBrands.length > 6 && (
                        <div className="relative mb-2">
                          <input
                            ...
                            className="w-full rounded-lg border border-gray-200 bg-[#F9FAFB] py-1 pl-7 pr-2 text-[11px] text-gray-900 outline-none focus:border-black"
                          />
                          ...
                          {brandSearchQuery && (
                            <button
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 hover:text-black font-bold"
                            >
                              ✕
                            </button>
                          )}
                      ...
                      <div className="space-y-1.5">
                        {(showAllBrands ? visibleBrands : visibleBrands.slice(0, 6)).map(brand => {
                      ...
                      {/* Titan-Style "See More v / See Less ^" */}
                      {visibleBrands.length > 6 && (
                        <button
                          type="button"
                          onClick={() => setShowAllBrands(prev => !prev)}
                          className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-neutral-800 hover:text-black hover:underline cursor-pointer pt-1"
                        >
                          <span>{showAllBrands ? 'See Less' : `See More (${visibleBrands.length - 6})`}</span>
                ...
                <div className="p-4">
                  ...
                  >
                    ...
                    <span className="text-gray-400 text-sm">
                      {openSections.categories ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                    </span>
                  </button>
                      ...
                      {categories.map(cat => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center justify-between rounded-lg px-2.5 py-1.5 text-xs font-semibold transition cursor-pointer ${
                              isSelected
                          ...
                          >
                            ...
                            <span
                              className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                                isSelected ? 'bg-white/20 text-amber-300' : 'text-gray-400'
                              }`}
                            >
                              {count}
                            </span>
                ...
                {/* 3. PRODUCT TYPE / SUBCATEGORIES ACCORDION (if available) */}
                {dynamicSubcategories.length > 1 && (
                  <div className="p-4">
                    ...
                    >
                      ...
                      <span className="text-gray-400 text-sm">
                        {openSections.subcategories ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                      </span>
                    </button>
                        ...
                        {dynamicSubcategories.map(subcat => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between rounded-lg px-2 py-1 text-xs transition cursor-pointer ${
                                isSelected
                ...
                {/* 4. PRICE ACCORDION (Brackets & Range Inputs) */}
                <div className="p-4">
                  <button
                    type="button"
                  ...
                  {openSections.price && (
                    <div className="mt-3 space-y-2">
                      <div className="space-y-1">
                        {PRICE_BRACKETS.map(bracket => (
                          <button
                            ...
                            className={`w-full text-left rounded-lg px-2.5 py-1 text-xs font-semibold transition cursor-pointer flex items-center justify-between ${
                      ...
                      <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-gray-100">
                        <div>
                          <label className="text-[9px] font-bold text-gray-400 block mb-0.5 uppercase">
                            Min (₹)
                          </label>
                          <input
                            type="number"
                            ...
                            className="w-full rounded-lg border border-gray-200 bg-[#F9FAFB] px-2 py-1 text-xs font-mono text-gray-900 outline-none focus:border-black"
                          />
                        </div>
                        <div>
                          <label className="text-[9px] font-bold text-gray-400 block mb-0.5 uppercase">
                ...
                <div className="p-4">
                  ...
                  >
                    <span>Discount</span>
                    <span className="text-gray-400 text-sm">
                      {openSections.discount ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                    </span>
                  </button>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-1">
                      {[
                        ...
                        <button
                          ...
                          className={`rounded-lg py-1 px-2 text-[11px] font-bold text-center transition cursor-pointer ${
                            minDiscount === disc.val
                ...
                <div className="p-4">
                  ...
                  >
                    ...
                    <span className="text-gray-400 text-sm">
                      {openSections.rating ? <Minus className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                    </span>
                  </button>
          ...
          <section className="min-w-0">
            ...
            {/* Header info matching Titan reference: "2,554 items | Men's Watches For Every Style And Occasion" */}
            <div className="mb-3 flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1">
              <h1 className="text-xs sm:text-sm font-semibold text-gray-500">
                <strong className="text-gray-950 font-extrabold text-sm sm:text-base">
                  {totalItems.toLocaleString('en-IN')} items
                </strong>{' '}
                <span className="text-gray-300">|</span>{' '}
                <span className="text-gray-800">
                  {category === 'All'
                    ? 'Curated Luxury Collection for Every Style & Occasion'
                    : `${category} Collection for Every Style & Occasion`}
                  {selectedBrand !== 'All' && ` • ${selectedBrand}`}
                </span>
              </h1>
            </div>

            {/* Promotional Banner Strip (Titan / XYLYS Style) */}
            <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[#0E1118] via-[#1A1E29] to-[#0A0C10] p-4 sm:p-5 text-white shadow-xs mb-4 border border-gray-800">
              <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="rounded bg-amber-400/20 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-amber-300 border border-amber-300/30">
                      Krishna Atelier
                    </span>
                  ...
                  <h2 className="text-sm sm:text-base font-black tracking-tight text-white uppercase">
                    Urban Flair <span className="text-amber-400">×</span> Swiss &amp; Japanese Precision
                  </h2>
                </div>

                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[11px] font-semibold text-gray-300 hidden md:inline">
                    Pan-India Insured Dispatch
                  </span>
                  <button
                    type="button"
                    onClick={() => handleCategorySelect('Watches')}
                    className="rounded-full bg-white px-3.5 py-1.5 text-xs font-bold text-gray-950 transition hover:bg-amber-300 cursor-pointer"
                  >
            ...
            {/* Fast Delivery Highlight Strip (Titan reference style) */}
            <div className="mb-4 flex items-center justify-between rounded-xl border border-amber-200/70 bg-[#FFFDF7] p-2.5 sm:px-4 text-xs text-amber-950">
              <div className="flex items-center gap-2.5">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-amber-100 text-amber-900 shrink-0">
                  <Zap className="w-4 h-4 text-amber-600 fill-amber-500" />
                </div>
                <div>
                  <span className="font-bold">Next Day Express Delivery</span>
                  <p className="text-[11px] text-gray-500 hidden sm:block">
                    Orders dispatched within 24 hours via BlueDart Air Express
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-600">
                <span className="hidden md:inline">Verified Authentic</span>
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
              </div>
            </div>

            {/* Top Toolbar: Search Bar + View Switcher + Mobile Filter Button */}
            <div className="mb-4 flex flex-col gap-2.5 rounded-xl border border-gray-200/90 bg-white p-2.5 sm:p-3 shadow-2xs sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="relative flex-1">
                <input
                  ...
                  placeholder={`Search ${category === 'All' ? 'products' : category}, brands, specifications...`}
                  className="w-full rounded-lg border border-gray-200 bg-[#F9FAFB] py-1.5 pl-8 pr-8 text-xs text-gray-900 placeholder:text-gray-400 outline-none transition focus:border-black focus:bg-white"
                />
                ...
                {search && (
                  <button
                    type="button"
                    onClick={() => setSearch('')}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 hover:text-black font-bold p-1 cursor-pointer"
                  >
                    ✕
                  </button>
                )}
              ...
              {/* View Mode Switcher + Mobile Filter Button */}
              <div className="flex items-center justify-between sm:justify-end gap-2">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 rounded-lg border border-gray-300 bg-[#F4F4F6] px-3 py-1.5 text-xs font-bold text-gray-900 lg:hidden hover:bg-gray-200 transition cursor-pointer shrink-0"
                >
                ...
                <div className="block lg:hidden">
                  <select
                    value={sort}
                    onChange={e => handleSortChange(e.target.value)}
                    className="rounded-lg border border-gray-200 bg-[#F9FAFB] px-2 py-1.5 text-xs font-semibold text-gray-900 outline-none"
                  >
                ...
                <div className="hidden sm:flex items-center p-0.5 rounded-lg bg-[#F4F4F6] border border-gray-200">
                  <button
                    type="button"
                ...
                <div className="hidden md:flex items-center gap-1 text-[11px] text-gray-500">
                  ...
                  {[9, 18, 27].map(num => (
                    <button
                      ...
                      className={`px-1.5 py-0.5 rounded font-mono text-[11px] font-bold transition cursor-pointer ${
                        itemsPerPage === num
                          ? 'bg-black text-white'
                          : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                      }`}
            ...
            {activeFiltersCount > 0 && (
              <div className="mb-4 flex flex-wrap items-center gap-1.5 rounded-xl border border-gray-200 bg-white p-2.5 text-xs shadow-2xs animate-fade-in">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mr-1">
                  Active Filters:
                </span>

                {category !== 'All' && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-900 border border-gray-200">
                ...
                {selectedSubcategory !== 'All' && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-900 border border-amber-200">
                ...
                <button
                  type="button"
                  onClick={clearAllFilters}
                  className="ml-auto inline-flex items-center gap-1 text-[11px] font-bold text-rose-600 hover:underline cursor-pointer"
                >
            ...
            {isFiltering ? (
              ...
              <>
                ...
                {viewMode === 'list' && (
                  <div className="space-y-3.5">
                    {paginatedProducts.map(product => (
                      <div
                        key={product.id}
                        className="group flex flex-col sm:flex-row items-center justify-between rounded-xl border border-gray-200 bg-white p-4 shadow-xs transition hover:shadow-md hover:border-gray-300 gap-4"
                      >
                        {/* Image */}
                        <Link
                          to={`/product/${product.id}`}
                          className="relative h-32 w-32 shrink-0 overflow-hidden rounded-lg bg-[#F4F3F0] p-1 flex items-center justify-center"
                        >
                          <img
                ...
                <div className="mt-8 rounded-xl border border-gray-200 bg-white p-5 text-center shadow-xs space-y-4">
                  {/* Progress Bar Indicator */}
                  <div className="max-w-xs mx-auto space-y-1.5">
                    <div className="flex justify-between text-[11px] font-bold text-gray-500">
                      <span>
                        Showing {Math.min(paginatedProducts.length, totalItems)} of {totalItems} items
                      </span>
                      <span>
                        {Math.round((Math.min(paginatedProducts.length, totalItems) / totalItems) * 100)}%
                      </span>
                    </div>
                    <div className="h-1.5 w-full rounded-full bg-gray-100 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(Math.min(paginatedProducts.length, totalItems) / totalItems) * 100}%`
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
                    {paginatedProducts.length < totalItems && (
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full bg-[#111827] px-7 py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-black shadow-sm transition active:scale-95 cursor-pointer"
                      >
              ...
              <div className="flex min-h-[360px] flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white p-8 sm:p-12 text-center shadow-xs">
                ...
                <p className="mt-1 max-w-md text-xs text-gray-500 leading-relaxed">
                  We couldn't find any products matching your selected criteria. Try adjusting your
                  brand, price range, or clearing filters.
                </p>
                <button
                  type="button"
                  onClick={clearAllFilters}
        ...
        {/* ================= LUXURY TRUST FOOTER STRIP ================= */}
        <div className="mt-12 rounded-2xl border border-gray-200 bg-white p-6 shadow-xs">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div className="space-y-1.5">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 text-[#B89758] border border-amber-200">
              ...
              <p className="text-[11px] text-gray-500">
                Rigorous boutique quality check with full product documentation.
              </p>
            </div>

            <div className="space-y-1.5">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-700 border border-blue-200">
                <TruckIcon className="w-5 h-5" />
              </div>
              <h4 className="text-xs font-black text-gray-950 uppercase tracking-wider">
                Insured BlueDart Delivery
              </h4>
              <p className="text-[11px] text-gray-500">
                Tamper-evident sealed packaging dispatched within 24 hours.
              </p>
            </div>

            <div className="space-y-1.5">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200">
              ...
              <p className="text-[11px] text-gray-500">
                Hassle-free reverse courier pickup and prompt refund settlement.
              </p>
            </div>

            <div className="space-y-1.5">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-purple-50 text-purple-700 border border-purple-200">
              ...
              <p className="text-[11px] text-gray-500">
                Safe UPI, Cards, NetBanking, and Cash on Delivery options.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-xs">
          <div className="text-center max-w-xl mx-auto mb-6">
            <span className="text-[10px] font-black uppercase tracking-widest text-[#B89758]">
              Client Assistance
            </span>
            <h3 className="text-lg font-black text-gray-950 mt-1">Frequently Asked Questions</h3>
      ...
      {mobileFilters && (
        <div className="fixed inset-0 z-50 flex justify-end lg:hidden">
          ...
          <div className="relative w-full max-w-xs sm:max-w-sm h-full bg-white shadow-2xl flex flex-col justify-between z-50 animate-slide-up">
            ...
            {/* Header */}
            <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3.5 bg-[#FAF9F6]">
              <div className="flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs divide-y divide-gray-100">
              ...
              {/* Brands */}
              <div className="pt-2">
                <h4 className="text-xs font-black uppercase tracking-wider text-gray-950 mb-2">
                  Brands
                </h4>
                    ...
                    return (
                      <label
                        ...
                        className={`flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-1.5 text-xs transition ${
                          isSelected
                            ? 'bg-amber-50 font-bold text-amber-950 border border-amber-300'
                            : 'text-gray-600 hover:bg-gray-50'
                        }`}
                      >
                        <div className="flex items-center gap-2 truncate">
                          ...
                          />
                          <span className="truncate">{brand === 'All' ? 'All Brands' : brand}</span>
                        </div>
                        <span className="text-[10px] font-mono text-gray-400">({count})</span>
                      </label>
                    );
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
                  {categories.map(cat => (
                    <button
                      ...
                      key={cat}
                      onClick={() => handleCategorySelect(cat)}
                      className={`flex w-full items-center justify-between rounded-lg px-3 py-1.5 text-xs font-semibold transition ${
                        category === cat
