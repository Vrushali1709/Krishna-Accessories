...
export default function Shop() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] text-gray-900 overflow-x-clip font-sans">
      <Navbar />

      {/* Floating Global Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-xl border border-gray-200 bg-white/95 px-4 py-3 text-xs font-semibold text-gray-950 shadow-xl backdrop-blur-md animate-slide-up">
          ...
          <Link
            to="/cart"
            className="ml-2 rounded-lg bg-gray-900 px-3 py-1 text-[11px] font-semibold text-white transition hover:bg-black"
          >
            View Bag &rarr;
          </Link>
        </div>
      ...
      <section className="border-b border-gray-200 bg-white py-5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mb-2">
            ...
            <button
              onClick={() => handleCategorySelect('All')}
              className={`hover:text-gray-900 transition ${category === 'All' ? 'font-semibold text-gray-900' : ''}`}
            >
              Shop
            </button>
            {category !== 'All' && (
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {totalItems} items available with express Pan-India delivery.
              </p>
            </div>
          </div>
        </div>
      ...
      <main ref={productsTopRef} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-8">
          ...
          {/* ================= LEFT SIDEBAR FILTERS (Clean Minimalist Style) ================= */}
          <aside className="hidden lg:block">
            <div className="sticky top-20 space-y-6 bg-white border border-gray-200 rounded-2xl p-5 shadow-2xs">

              {/* Sidebar Header */}
              <div className="flex items-center justify-between pb-3 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  <SlidersHorizontal className="w-4 h-4 text-gray-800" />
                  <span className="text-xs font-bold uppercase tracking-wider text-gray-900">Filters</span>
                </div>
                {activeFiltersCount > 0 && (
                  <button
                    type="button"
              ...
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-900 mb-2.5">Categories</h4>
                <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                  {categories.map((cat) => {
                    const isSelected = category === cat;
                    const count = cat === 'All'
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => handleCategorySelect(cat)}
                        className={`w-full flex items-center justify-between rounded-lg px-3 py-1.5 text-xs transition cursor-pointer ${isSelected
                            ? 'bg-gray-900 text-white font-semibold'
                            : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                          }`}
                      >
                        <span className="truncate">{cat}</span>
                        <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${isSelected ? 'bg-white/20 text-amber-300' : 'text-gray-400'}`}>
                          {count}
                        </span>
                      </button>
                    );
              ...
              {dynamicSubcategories.length > 1 && (
                <div className="border-t border-gray-100 pt-4">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-gray-900 mb-2.5">Product Type</h4>
                  <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
                    {dynamicSubcategories.map((subcat) => {
                      const isSelected = selectedSubcategory === subcat;
                      return (
              ...
              <div className="border-t border-gray-100 pt-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-gray-900">Brands</h4>
                  {selectedBrand !== 'All' && (
                    <button
                      type="button"
                      onClick={() => handleBrandSelect('All')}
                ...
                {dynamicBrands.length > 5 && (
                  <div className="mb-2">
                    <input
                      ...
                      className="w-full rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-1.5 text-xs text-gray-900 outline-none focus:border-gray-900"
                    />
                  </div>
                )}
                  ...
                  {visibleBrands.map((brand) => {
                    ...
                    return (
                      <label
                        key={brand}
                        onClick={() => handleBrandSelect(brand)}
                        className={`flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-1.5 text-xs transition ${isSelected ? 'bg-gray-100 font-semibold text-gray-950' : 'text-gray-600 hover:bg-gray-50'
                          }`}
                      >
                        <div className="flex items-center gap-2 truncate">
                          <input
                            ...
                            checked={isSelected}
                            onChange={() => handleBrandSelect(brand)}
                            className="accent-gray-900 h-3 w-3"
                          />
                          <span className="truncate">{brand}</span>
                        </div>
                      </label>
              ...
              {/* 4. Price Tiers */}
              <div className="border-t border-gray-100 pt-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-gray-900 mb-2.5">Price Range</h4>
                <div className="space-y-1">
                  {PRICE_BRACKETS.map((bracket) => (
                    <button
                      key={bracket.id}
                      type="button"
                      onClick={() => handlePriceBracketSelect(bracket)}
                      className={`w-full text-left rounded-lg px-2.5 py-1.5 text-xs transition cursor-pointer flex items-center justify-between ${activePriceBracket === bracket.id
          ...
          <div className="min-w-0">
            ...
            <div className="mb-6 flex flex-col sm:flex-row gap-3 items-center justify-between bg-white border border-gray-200 p-3 rounded-2xl shadow-2xs">
              ...
              <div className="relative w-full sm:max-w-xs">
                <input
                  ...
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 pl-9 pr-8 text-xs text-gray-900 placeholder:text-gray-400 outline-none focus:border-gray-900 focus:bg-white"
                />
                <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-3.5 h-3.5" />
                {search && (
                  <button
                    type="button"
                    onClick={() => setSearch('')}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 hover:text-gray-900 font-bold"
                  >
                    ✕
                  </button>
                )}
              ...
              <div className="flex items-center justify-between w-full sm:w-auto gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setMobileFilters(true)}
                  className="flex items-center gap-1.5 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-900 lg:hidden hover:bg-gray-100"
                >
                  <SlidersHorizontal className="w-3.5 h-3.5" />
                  <span>Filters</span>
                  {activeFiltersCount > 0 && (
                ...
                {/* View Mode Switcher */}
                <div className="hidden sm:flex items-center p-0.5 rounded-xl bg-gray-100 border border-gray-200">
                  <button
                    type="button"
                    onClick={() => setViewMode('grid-3')}
                    className={`p-1.5 rounded-lg transition ${viewMode === 'grid-3' ? 'bg-white text-gray-900 shadow-2xs' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    <LayoutGrid className="w-3.5 h-3.5" />
                  </button>
                  <button
                    type="button"
                    onClick={() => setViewMode('grid-4')}
                    className={`p-1.5 rounded-lg transition ${viewMode === 'grid-4' ? 'bg-white text-gray-900 shadow-2xs' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    <Grid2X2 className="w-3.5 h-3.5" />
                  </button>
                  <button
                    type="button"
                    onClick={() => setViewMode('list')}
                    className={`p-1.5 rounded-lg transition ${viewMode === 'list' ? 'bg-white text-gray-900 shadow-2xs' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    <List className="w-3.5 h-3.5" />
                  </button>
                </div>
                ...
                <select
                  value={sort}
                  onChange={(e) => handleSortChange(e.target.value)}
                  className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-900 outline-none focus:border-gray-900 cursor-pointer"
                >
                  <option value="featured">Featured &amp; Best Match</option>
                  <option value="price-low">Price: Low to High</option>
                  <option value="price-high">Price: High to Low</option>
            ...
            {/* Active Filters Pill Bar */}
            {activeFiltersCount > 0 && (
              <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl bg-white border border-gray-200 p-3 text-xs">
                <span className="text-gray-400 font-semibold">Active:</span>
                {category !== 'All' && (
                  <span className="inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1 font-medium text-gray-800">
                    {category} <button onClick={() => handleCategorySelect('All')} className="font-bold hover:text-red-600">×</button>
                  </span>
                )}
                {selectedBrand !== 'All' && (
                  <span className="inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1 font-medium text-gray-800">
                    {selectedBrand} <button onClick={() => handleBrandSelect('All')} className="font-bold hover:text-red-600">×</button>
                  </span>
                )}
                {selectedSubcategory !== 'All' && (
                  <span className="inline-flex items-center gap-1 rounded-lg bg-amber-50 px-2.5 py-1 font-medium text-amber-900 border border-amber-200">
                    {selectedSubcategory} <button onClick={() => handleSubcategorySelect('All')} className="font-bold hover:text-red-600">×</button>
                  </span>
                )}
                <button onClick={clearAllFilters} className="ml-auto text-amber-700 font-semibold hover:underline">
                  Clear All
                </button>
              </div>
            )}
            ...
            {isFiltering ? (
              ...
              <>
                ...
                {viewMode === 'list' && (
                  <div className="space-y-3">
                    {paginatedProducts.map((product) => (
                      <div
                        key={product.id}
                        className="flex flex-col sm:flex-row items-center justify-between rounded-2xl border border-gray-200 bg-white p-4 shadow-2xs hover:border-gray-300 gap-4 transition"
                      >
                        <Link to={`/product/${product.id}`} className="h-28 w-28 shrink-0 bg-gray-50 rounded-xl p-2 flex items-center justify-center">
                          <img src={product.image || product.images?.[0]} alt={product.name} className="h-full w-full object-contain" />
                        </Link>
                        <div className="flex-1 text-center sm:text-left min-w-0">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-amber-700">{product.brand}</span>
                          <Link to={`/product/${product.id}`} className="block text-sm font-bold text-gray-900 truncate mt-0.5 hover:underline">
                            {product.name}
                          </Link>
                          <p className="text-xs text-gray-500 line-clamp-1 mt-1">{product.description}</p>
                        </div>
                        <div className="flex flex-col sm:items-end gap-2 w-full sm:w-auto">
                          <span className="text-base font-bold text-gray-950">₹{Number(product.price).toLocaleString('en-IN')}</span>
                          <button
                            type="button"
                            onClick={() => handleAddToCart(product)}
                            className="rounded-xl bg-gray-900 px-4 py-2 text-xs font-semibold text-white hover:bg-black transition cursor-pointer"
                          >
                            Add to Bag
                          </button>
                        </div>
                ...
                <div className="mt-8 flex items-center justify-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl border border-gray-200 bg-white text-xs font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-40"
                  >
                    Previous
                  </button>
              ...
              <div className="flex min-h-[300px] flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white p-8 text-center">
                ...
                <p className="text-xs text-gray-500 mt-1 mb-4">Try adjusting your filters or search criteria.</p>
                <button
                  type="button"
                  onClick={clearAllFilters}
      ...
      {mobileFilters && (
        <div className="fixed inset-0 z-50 flex justify-end lg:hidden">
          <div onClick={() => setMobileFilters(false)} className="fixed inset-0 bg-black/50 backdrop-blur-2xs" />
          <div className="relative w-full max-w-xs h-full bg-white shadow-2xl flex flex-col justify-between z-50">
            <div className="flex items-center justify-between border-b border-gray-200 p-4">
              ...
              <button onClick={() => setMobileFilters(false)} className="text-xs font-bold text-gray-500">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
              <div>
                <h4 className="font-bold uppercase tracking-wider text-gray-900 mb-2">Categories</h4>
                <div className="space-y-1">
                  {categories.map((cat) => (
                    <button
                      key={cat}
                      onClick={() => handleCategorySelect(cat)}
                      className={`w-full text-left px-3 py-2 rounded-lg font-medium ${category === cat ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
                      {cat}
                    </button>
                  ))}
            ...
            <div className="border-t border-gray-200 p-4 flex gap-2">
              <button onClick={clearAllFilters} className="flex-1 py-2.5 border border-gray-300 rounded-xl font-semibold text-xs">Clear</button>
              <button onClick={() => setMobileFilters(false)} className="flex-1 py-2.5 bg-gray-900 text-white rounded-xl font-semibold text-xs">Apply</button>
            </div>
          </div>
        </div>
      )}
