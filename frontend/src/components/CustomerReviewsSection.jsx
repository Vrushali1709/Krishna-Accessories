...
const DEFAULT_REVIEWS = [
  {
    ...
    rating: 5,
    product: 'Titan Edge Ceramic Chronograph',
    text: 'Ordered the Titan Edge Ceramic watch. The packaging was immaculate with the brand warranty card.',
    date: 'Verified Buyer • 2 days ago'
  },
  {
    id: 2,
    ...
    rating: 5,
    product: 'Hidesign Genuine Leather Tote',
    text: 'The quality of the leather bag is top-notch. Fast same-day pickup from their store.',
    date: 'Verified Buyer • 4 days ago'
  },
  {
    id: 3,
    ...
    rating: 5,
    product: 'Sony WH-1000XM5 Noise Cancelling',
    text: 'Got the Sony flagship headphones at a fantastic price using code KRISHNA10.',
    date: 'Verified Buyer • 1 week ago'
  }
];
...
export default function CustomerReviewsSection({ reviews = DEFAULT_REVIEWS }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 border-t border-neutral-200/60">
      <Reveal direction="up" delay={50}>
        <div className="flex items-end justify-between mb-8">
          <div>
            <span className="text-[11px] font-mono tracking-[0.2em] uppercase text-neutral-400 block mb-1">
              TESTIMONIALS
            </span>
            <h2 className="text-2xl sm:text-3xl font-light tracking-tight text-neutral-900">
              Verified Client <span className="font-semibold">Feedback</span>
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => scrollCarousel('left')}
              disabled={!canScrollLeft}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-neutral-200 bg-white text-neutral-700 disabled:opacity-30 cursor-pointer"
            >
              <ChevronLeftIcon className="w-4 h-4" />
            </button>
            <button
      ...
      <Reveal direction="up" delay={100}>
        ...
        >
          {reviews.map((review, idx) => (
            <div
              key={review.id || idx}
              className="flex-shrink-0 w-[290px] sm:w-[340px] snap-start flex flex-col justify-between rounded-2xl border border-neutral-200/80 bg-white p-5"
            >
              <div>
              ...
              <div className="pt-3 border-t border-neutral-100 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-semibold text-neutral-900">{review.name}</h4>
                  <p className="text-[10px] text-neutral-400">{review.location}</p>
                </div>
                <span className="h-7 w-7 rounded-full bg-neutral-100 text-[10px] font-mono font-medium flex items-center justify-center text-neutral-700">
                  {review.name.charAt(0)}
                </span>
              </div>
            </div>
