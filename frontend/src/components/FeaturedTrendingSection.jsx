...
import { isInWishlist, toggleWishlist } from '../utils/productStore';
import { Reveal } from './useScrollReveal';
import { HeartIcon, BagIcon, StarIcon, CheckIcon } from './Icons';
...
export default function FeaturedTrendingSection({ products = [], onToast }) {
  ...
  const tabs = [
    ...
    { id: 'best-deals', label: 'Special Deals' }
  ];
  ...
  return (
    <section className="bg-white py-12 sm:py-16 border-t border-b border-neutral-200/60">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
            <div>
              <h2 className="text-2xl sm:text-3xl font-light tracking-tight text-neutral-900">
                Curated <span className="font-semibold">Collection</span>
              </h2>
              <p className="mt-1 text-xs sm:text-sm text-neutral-500 max-w-lg">
                Handpicked timepieces, premium leather items, and modern tech gear.
              </p>
            </div>

            {/* Feature Tabs */}
            <div className="flex items-center gap-1.5 p-1 bg-neutral-100 rounded-xl w-full lg:w-auto overflow-x-auto">
              {tabs.map((tab) => {
                const isActive = activeTab === tab.id;
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setActiveTab(tab.id)}
                    className={`px-4 py-2 rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer whitespace-nowrap ${
                      isActive
            ...
            {categoryFilters.map((cat) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setSelectedCategory(cat)}
                  className={`shrink-0 px-3.5 py-1.5 text-xs rounded-full border transition-all duration-200 cursor-pointer ${
                    isActive
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {filteredProducts.map((product, idx) => {
              ...
              return (
                <Reveal key={`feat-${product.id}`} direction="up" delay={(idx % 4) * 60}>
                  <div className="group flex flex-col justify-between h-full rounded-2xl border border-neutral-200/70 bg-white p-3.5 transition-all duration-300 hover:border-neutral-300 hover:shadow-[0_10px_25px_rgba(0,0,0,0.04)]">
                    ...
                    <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-neutral-100 mb-3">
                      ...
                      <div className="absolute top-2.5 inset-x-2.5 flex items-center justify-between pointer-events-none">
                        {discount > 0 ? (
                          <span className="rounded-md bg-neutral-900 text-white px-2 py-0.5 text-[9px] font-mono tracking-wider">
                            -{discount}%
                          </span>
                        ) : (
                          <span />
                        ...
                        <button
                          ...
                          onClick={(e) => handleWishlistToggle(e, product)}
                          aria-label={isWish ? 'Remove from wishlist' : 'Add to wishlist'}
                          className={`pointer-events-auto flex h-7.5 w-7.5 items-center justify-center rounded-full backdrop-blur-md shadow-xs transition-all duration-200 cursor-pointer ${
                            isWish
                    ...
                    <div className="flex flex-1 flex-col justify-between">
                      <div>
                        ...
                        <Link
                          to={`/product/${product.id}`}
                          className="block text-sm font-medium text-neutral-900 transition-colors line-clamp-1 hover:text-neutral-600 mb-2"
                        >
                          {product.name}
                        </Link>
                      ...
                      <div className="flex items-center gap-2 pt-2 border-t border-neutral-100">
                        <button
                          type="button"
                          onClick={(e) => handleAddToCart(e, product)}
                          className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
                            isAdded
                        ...
                        <button
                          type="button"
                          onClick={(e) => handleBuyNow(e, product)}
                          className="px-3 py-2 rounded-lg text-xs font-medium border border-neutral-300 text-neutral-700 hover:bg-neutral-50 transition-colors cursor-pointer"
                        >
                          Buy
                        </button>
                      </div>
          ...
          <div className="text-center py-12 rounded-xl bg-neutral-50 border border-neutral-200/60">
            <p className="text-xs font-medium text-neutral-500">No items found matching the selected parameters.</p>
          </div>
        )}
      </div>
    </section>
