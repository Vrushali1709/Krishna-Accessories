...
const PRICE_TIERS = [
  { id: 'under-5k', maxPrice: 5000, minPrice: 0, label: 'Under ₹5,000', note: 'Everyday picks' },
  { id: 'under-10k', maxPrice: 10000, minPrice: 0, label: 'Under ₹10,000', note: 'Most ordered' },
  { id: 'under-15k', maxPrice: 15000, minPrice: 0, label: 'Under ₹15,000', note: 'Step up' },
  { id: 'under-20k', maxPrice: 20000, minPrice: 0, label: 'Under ₹20,000', note: 'The good stuff' }
];

export default function ProductsByPriceSection({ products = [], onAddToCart, onBuyNow }) {
  const navigate = useNavigate();
  const [activeTierId, setActiveTierId] = useState('under-5k');

  const activeTier = useMemo(
    () => PRICE_TIERS.find((t) => t.id === activeTierId) || PRICE_TIERS[0],
    [activeTierId]
  );
  ...
  const activeProducts = useMemo(() => {
    return products
      .filter(
        (p) =>
          Number(p.price) <= activeTier.maxPrice && Number(p.price) >= activeTier.minPrice
      )
      .sort((a, b) => {
        const scoreB = (Number(b.rating) || 4.5) * 50 + (Number(b.reviews) || 10);
        const scoreA = (Number(a.rating) || 4.5) * 50 + (Number(a.reviews) || 10);
      ...
      <div className="wrap">
        ...
        <div className="border-b border-line pb-4">
          ...
          <p className="mt-1 max-w-[60ch] text-[13px] text-ash">
            Set a ceiling and see the best things under it.
          </p>
        </div>
        ...
        <div
          ...
          className="-mx-5 mt-6 flex gap-2 overflow-x-auto px-5 no-scrollbar sm:mx-0 sm:grid sm:grid-cols-4 sm:gap-3 sm:px-0"
        >
          {PRICE_TIERS.map((tier) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setActiveTierId(tier.id)}
                className={`min-w-[150px] shrink-0 rounded-[3px] border px-4 py-3.5 text-left transition-colors ${
                  isActive
              ...
              >
                ...
                <span
                  className={`num mt-0.5 block text-[11.5px] ${
                    isActive ? 'text-neutral-300' : 'text-ash'
                  }`}
                >
                  {count > 0 ? `${count} items` : tier.note}
                </span>
              </button>
            );
          })}
        ...
        {activeProducts.length > 0 ? (
          <>
            <div className="mt-10 grid grid-cols-2 gap-x-4 gap-y-10 lg:grid-cols-4 lg:gap-x-6">
              {activeProducts.map((product) => (
                <ProductCard
                  key={`price-${activeTier.id}-${product.id}`}
                  product={product}
                  onAddToCart={handleQuickAdd}
                  onBuyNow={handleInstantBuy}
                  showRating={true}
            ...
            <div className="mt-10 border-t border-line pt-5">
              <Link
                to={`/shop?maxPrice=${activeTier.maxPrice}`}
                className="link-ul text-[13px] font-medium"
              >
                See all {tierCounts[activeTier.id] || 0} products {activeTier.label.toLowerCase()}
              </Link>
            </div>
          </>
        ) : (
          <div className="mt-10 border border-line bg-paper px-6 py-16 text-center">
            <p className="text-[14px] text-ash">
              Nothing in stock {activeTier.label.toLowerCase()} right now.
            </p>
            <Link to="/shop" className="btn btn-line btn-sm mt-4">
