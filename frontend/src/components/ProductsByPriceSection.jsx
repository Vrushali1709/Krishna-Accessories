...
export default function ProductsByPriceSection({ products = [], onAddToCart, onBuyNow }) {
  ...
  const handleQuickAdd = (product) => {
    ...
    else addToCart(product, 1, product.colors?.[0] || '', product.variants?.[0] || '');
  };
  ...
  return (
    <section className="bg-[#FAF9F6] py-14 border-t border-neutral-200/60">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-light tracking-tight text-neutral-900">
                Shop by <span className="font-semibold">Budget</span>
              </h2>
            </div>
              ...
              {PRICE_TIERS.map((tier) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setActiveTierId(tier.id)}
                    className={`px-4 py-2 rounded-xl text-xs font-medium transition-all duration-200 cursor-pointer whitespace-nowrap ${
                      isActive
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
          {activeProducts.map((product, idx) => (
            <Reveal key={`price-${product.id}`} direction="up" delay={idx * 60}>
              <ProductCard
                product={product}
                onAddToCart={handleQuickAdd}
                onBuyNow={handleInstantBuy}
        ...
        <div className="flex justify-end">
          ...
          >
            <span>Explore all items {activeTier.label}</span>
